...
exports.getHistoryDateFilter = getHistoryDateFilter;
 ...
 */
async function enforceFeature(orgId, feature, currentCount) {
    ...
    if (actualCurrent === undefined) {
        switch (feature) {
            case "TEMPLATE_COUNT":
                actualCurrent = await prisma_1.prisma.template.count({
                    where: { orgId },
                });
                break;
            case "BRAND_VOICE_COUNT":
                actualCurrent = await prisma_1.prisma.brandVoice.count({
                    where: { orgId },
                });
                break;
            case "HISTORY_DAYS":
    ...
    if (actualCurrent >= limit) {
        const err = new Error(`Feature limit exceeded: ${feature}`);
 ...
 */
async function getHistoryDateFilter(orgId) {
    const org = await prisma_1.prisma.organization.findUnique({
        where: { id: orgId },
        select: {
            id: true,
